...
const resources = {
  en: {
    translation: {
      ...
      FAQ_TdNotFound_Answer:'For each class, there is a TD group associated. We use this to enable you to \'subscribe\' to these classes. But sometimes, as these groups are written by hand, there can be some errors and we can\'t do anything on our side. We are actively working on a solution (temporary or permanent).',
      FAQ_AssoPublisher_Question:'How can I publish events ?',
      FAQ_AssoPublisherAnswer:'If you are eligible (an association, a club, or an INSA Service for exemple) you can contact the DSI to be flagged as a publisher. You must have beforehand visited the website at least once (for your account to be created via CAS) and there must be someone legally responsible for the CAS account you\'ll be using. When all this is done, there will be an extra tab in the Settings page for you to create and publish an event.',
      FAQ_Webview_Question:'Is the calendar available to download ?',
      FAQ_Webview_Answer:'The app is not available on Google Play or App Store but you can create a webview for yourself, as most navigators allow you to add a page to your homescreen. The way to do this can vary depending on your phone\'s model and the version of your navigator.',
      FAQ_FriendNotFound_Question:'I can\'t find my friends in the research bar',
      FAQ_FriendNotFound_Answer:'First, check if you used the username of your friend and not their full name (Alice Durand would be adurand). Then, if you still can\'t find your friend, even with the right username, maybe they never connected to our website. The users are created when first connecting with CAS so they must first have visited they app at least once before they are visible in the list',
      FAQ_WhyDuck_Question:'Why are there duck everywhere ?',
      FAQ_WhyDuck_Answer:'Ducks are the emblem of the ITI department. This comes from a story every developper know. When you are stuck with a problem and you can\'t find the solution, it often helps to talk about it with someone. And as many people know ITI student don\'t have friends, so we use a rubber duck instead X) .', 
    },
  },
  fr: {
    translation: {
      ...
      FAQ_FriendNotFound_Question:'Je n\'arrive pas à trouver mes amis dans la barre de recherche',
      FAQ_FriendNotFound_Answer:'Tout d\'abord, vérifiez que vous avez bien utilisé le nom d\'utilisateur de votre ami et non son nom complet (Alice Durand serait adurand). Ensuite, si vous ne trouvez toujours pas votre ami, même avec le bon nom d\'utilisateur, il se peut qu\'il ne se soit jamais connecté à notre site web. Les utilisateurs sont créés lors de leur première connexion à CAS, ils doivent donc avoir visité l\'application au moins une fois avant d\'apparaître dans la liste.',
      FAQ_WhyDuck_Question:'Pourquoi il y a des canards partout ?',
      FAQ_WhyDuck_Answer:'Les canards sont l\'emblème du département ITI. Ça vient d\'une histoire que tous les développeurs connaissent. Lorsque vous êtes bloqué sur un problème et que vous ne trouvez pas la solution, il est souvent utile d\'en parler à quelqu\'un. Et comme beaucoup le savent, les ITI n\'ont pas d\'amis, alors on utilise un canard en plastique à la place X) .', 
    },
  },
};
